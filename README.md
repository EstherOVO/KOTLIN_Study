...
## :pushpin: Enum Class
- 고정된 상수 집합
- 각 항목이 Enum 클래스의 인스턴스로 존재한다.
- 정해진 값들 중에서만 선택할 수 있도록 강제한다.
- 프로퍼티와 메서드를 가질 수 있다.
...
- 같은 패키지 내부에서만 하위 클래스 상속이 가능하다.
- 컴파일 시 하위 클래스의 타입을 모두 기억한다.
- 제한적으로 상속하고 유연하지만 제어된 확장을 가능하게 한다.
- `when` 문과 함께 사용 시 모든 경우가 처리되었는지 컴파일 시점에 검사한다.
- **상태에 따른 특정 로직을 구현할 때 유용**
- `sealed interface`도 선언 가능(봉인된 클래스보다 더 유연하게 다중 상속 가능)

# ▨ 여러 가지 함수(Extension Function) ▨
## :pushpin: 확장 함수(Extension Function)
...
## :pushpin: 확장 프로퍼티(Extension Property)
...
## :pushpin: 중위 함수(Infix Notation)
- 메서드의 호출을 `infix` 키워드를 통해 자연스러운 방식으로 표현할 수 있게 하는 문법 기능
- 두 개의 파라미터 → 하나는 메서드의 수신 객체, 하나는 인자
- 중위 함수는 반드시 하나의 매개 변수를 가져야 한다.
- 자연 언어에 가까운 코드 스타일을 만들 수 있다.

## :pushpin: 인라인 함수(Inline Function)
- 함수 성능 최적화를 위해 사용되는 함수
- `inline` 키워드가 있으면 함수 호출하는 대신 함수 본문으로 대체한다.
- (내부적으로) 새로운 객체가 생성되거나, 함수가 호출이 되는 방식이 아니기 때문에 메모리 간접 비용과 성능 비용을 줄인다.
- 주로 람다 표현식을 매개변수로 사용한다.
- 코드가 호출 지점에 복제된다.

## :pushpin: 지역 함수(Local Function)
- 다른 함수 내부에 정의된 함수
- 자바의 경우 함수 내부에 함수를 정의할 수 없다.
- 코틀린은 함수 안에 함수를 사용할 수 있으며, 자신이 정의된 함수 내부에서만 사용할 수 있다.
- 코드를 블럭화 시키고 밀접하게 연관된 로직을 여러번 내부에서 사용해야 할 경우나 주요 로직과의 분리가 필요할 경우 사용할 수 있다.

# ▨ 람다(Lambda) ▨
## :pushpin: 람다(함수형 프로그래밍, FP)
...
- 자바에서는 함수(Function)가 일급 시민이 아닌 2급 시민
  - 변수에 할당되거나 매개변수로 전달되거나 리턴 값으로 받을 수 없다.
- 코틀린에서는 함수가 그 자체로 값이 되고, 변수에 할당되고, 매개변수로 넘기고, 리턴 값으로 받을 수 있다.

## :pushpin: 고차 함수(Higher Order Function)
- 함수의 인자로 함수를 전달하거나 함수의 결과로 함수를 반환하는 함수
- 함수라는 개체가 일급 시민일 때 가능한 함수
- 함수형 프로그래밍 패러다임을 가능하게 한다.

## :pushpin: 람다 표현식(Lambda Expression)
- 고차 함수에 인자로 정달되거나 결과로 반환할 수 있는 익명 함수를 간결하게 표현한 문법
- 코틀린에서 람다 표현식은 `{ }` 안에 정의
- `{ 매개변수 -> 코드 본문 }`
- 매개변수 : 람다로 전달되는 입력값
  - 매개변수가 없을 경우에 생략 가능
...
- 화살표(`->`) : 파라미터와 코드 본문을 구분
  - 매개변수가 없거나 하나일 때 생략 가능하다.
- 코드본문 : 람다 실행 코드 블럭
  - 본문의 마지막 줄의 표현식(Expression)이 리턴 값

## :pushpin: 클로저
- 람다 표현식이나 익명 함수가 외부 범위(Scope)에 있는 변수를 캡쳐해서 참조할 수 있는 기능
- 캡쳐된 변수는 해당 범위(Scope)를 벗어나더라도 캡쳐된 변수에 접근하고 조작할 수 있다.
- Closure : 람다가 시작되는 지점에 참조하고 있는 변수를 모두 캡쳐하여 정보를 가진다.

# ▨ 컬렉션 프레임워크(Collection Framework) ▨
## :pushpin: 컬렉션 프레임워크 함수형 프로그래밍
1. `map` : 변환
2. `filter` : 추출
3. `reduce` : 누적 연산
4. `sortedBy` : 정렬
...
## :pushpin: 스코프 함수(Scope Function)
- 일시적인 영역(Scope)을 형성하는 함수
- 람다 표현식 내부에 수신 객체에 쉽게 접근할 수 있는 방법을 제공
- 종류
  1. `let` : 수신객체 `it`, 리턴 값 → 람다 블럭의 결과 값(마지막 행)
  2. `also` : 수신객체 `it`, 리턴 값 → 수신객체 자체
  3. `run` : 수신객체 `this` 리턴 값 → 람다 블럭의 결과 값(마지막 행)
  4. `apply` : 수신객체 `this`,  리턴 값 → 수신객체 자체
  5. `with` : 객체를 인자로 받고, `this` 키워드, 람다 결과 값 리턴

# ▨ 지연 초기화(Lazy Initialization) ▨
## :pushpin: 지연 초기화(Lazy Initialization)
- 객체나 변수를 실제로 필요할 때까지 초기화하지 않는 것
- 초기화 시점을 미룰 수 있고, 메모리 리소스를 절약할 수 있다.
1. lateinit
   - 프로퍼티 타입 : `var`(가변)
   - 타입 제한 : 기본타입(Primitive 타입)과 Nullable 타입 불가
   - 초기화 시점을 정확히 알 수 없는 경우 사용한다.
2. lazy
   - 프로퍼티 타입 : `val`(불변)
   - 타입 제한 : 제한 없음
   - 초기화 비용이 든 경우 사용한다.