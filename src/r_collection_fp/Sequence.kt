...
// 시퀀스(Sequence) : 지연 평가를 사용하여 데이터 컬렉션을 효율적으로 처리
// 자바의 Stream API에 대응
fun main() {

//  시퀀스 생성

//  1. sequenceOf 를 통한 생성
...
//  컬렉션 연산
//  즉시 평가(eager evolution) : 중간연산(filter, map)을 수행할 때마다 새로운 컬렉션을 반환
    val collectionTime = measureTimeMillis {
        largeList.filter { it % 2 == 0 }    // 50만 개 리스트 반환
            .map { it * it }    // 50만 개 리스트 변환
...
//  시퀸스 연산
//  지연 평가(Lazy Evaluation) : 최종 연산이 실행될 때까지 대기하고 필요한 연산만 수행
    val sequenceTime = measureTimeMillis {
        largeList.asSequence().filter { it % 2 == 0 }   // 10개만 필터링
            .map { it * it }    // // 10개만 변환
            .take(10)   // 컬렉션에서 앞 10개만 가져옴
            .toList()   // 지연 연산
    }
