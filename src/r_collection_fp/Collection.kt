...
import p_extension.ex.startsWithChar
...
//  filter : 조건에 맞는 요소만 추출
    ...
    val odds = numbers.filter { it % 2 == 1 }   // 매개변수 1개면 it 키워드 사용
    println(odds)
...
//  fold : 초기 값을 직접 지정하고 누적 연산 수행
    val sumFold = numbers.fold(100) { acc: Int, i: Int -> acc + i }    // 100 + 1 + 2 + 3 + ... + 8
    println("sumFold = ${sumFold}")

//  groupBy : 기준에 따라 그룹화
    val words = listOf("apple", "banana", "applemango", "blueberry", "choco", "diamond", "clock")
    val groupByMap: Map<Char, List<String>> = words.groupBy { it.first() }    // 첫 번째 글자로 그룹화
    println("groupByMap = ${groupByMap}")
//  groupByMap = {a=[apple, applemango], b=[banana, blueberry], c=[choco, clock], d=[diamond]}
...
//  none : 컬렉션의 모든 요소가 조건을 충족하지 않으면 true
    val isNoneEven = odds.none { it % 2 == 0 }  // true
    println("isNoneEven = ${isNoneEven}")

//  any : 컬렉션의 하나의 요소라도 조건을 충족하면 true
    val isAnyEven = numbers.any { it % 2 == 0 } // true
